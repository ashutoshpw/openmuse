...
export const userMessagePartSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("file"), artifactId: idSchema, name: safeNameSchema.optional() }).strict(),
  z.object({ type: z.literal("image"), artifactId: idSchema, alt: z.string().max(500).optional() }).strict(),
  z.object({ type: z.literal("audio"), artifactId: idSchema, durationMs: z.number().int().nonnegative().max(86_400_000).optional() }).strict(),
  z.object({ type: z.literal("artifactRef"), artifactId: idSchema, purpose: z.string().trim().max(128).optional() }).strict(),
...
export const changeGoalStatusInputSchema = z.object({
...
export const createConnectionIntentInputSchema = z.object({
  workspaceId: idSchema,
  app: z.enum(["gmail", "calendar"]),
  returnPath: z.string().trim().max(2048).optional(),
}).strict();

export const completeConnectionIntentInputSchema = z.object({
  intentId: idSchema,
...
export const revokeShareInputSchema = z.object({ reason: z.string().trim().max(500).optional() }).strict();
