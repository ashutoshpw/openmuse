...
describe("core provider registry", () => {
  ...
  it("waits for creation while closing and rejects new resolutions", async () => {
    ...
    registry.register(driver);
    const scope = registry.createScope();
    const resolving = scope.resolve(
      "closing-instance",
      "model",
      "fake-model",
      { defaultModel: "fake" },
      { configDigest: "closing" },
    );
    ...
    await closing;
    await expect(
      scope.resolve("after-close", "model", "fake-model", { defaultModel: "fake" }),
...
describe("approval service", () => {
  it("binds approval to its digest and consumes it once", async () => {
    ...
    await expect(service.consume("approval-1", binding)).resolves.toMatchObject({
      status: "consumed",
    });
    await expect(service.consume("approval-1", binding)).rejects.toThrow(
  ...
  it("rejects changed execution arguments and expires before consumption", async () => {
    ...
    const binding = {
      ...
      runId: "run-2",
      toolCallId: "tool-2",
      canonicalArguments: { eventId: "event-2" },
      targetResources: { calendarId: "calendar-1" },
      policyVersion: "v1",
    };
    ...
    await expect(
      service.consume("approval-2", { ...binding, canonicalArguments: { eventId: "changed" } }),
