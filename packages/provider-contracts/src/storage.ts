...
export interface StorageConfig { endpoint?: string; bucket?: string }
export interface StoragePutRequest { key: string; blob: ProviderBlob; metadata?: Record<string, string> }
export interface StorageObject { key: string; contentType: string; sizeBytes: number; sha256?: string }
export interface StorageClient extends AsyncDisposable {
  ...
  createDownloadUrl(key: string, expiresInSeconds: number, context: ProviderOperationContext): Promise<string>;
}
